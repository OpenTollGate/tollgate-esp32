...
static const char *TAG = "tollgate_core_mining";

static tollgate_mining_client_stats_t s_clients[MINING_MAX_CLIENTS];
...
{
    uint64_t diff = tollgate_core_mining_nbits_to_difficulty(nbits);
...
uint64_t tollgate_core_mining_shares_to_allotment_ms(double hashrate_ghs, double hashprice_sats_per_ghs_s,
...
uint64_t tollgate_core_mining_shares_to_allotment_bytes(double hashrate_ghs, double hashprice_sats_per_ghs_s,
...
tollgate_mining_client_stats_t *tollgate_core_mining_get_or_create_client(uint32_t client_ip)
{
    ...
    if (s_client_count >= MINING_MAX_CLIENTS) {
        for (int i = 0; i < MINING_MAX_CLIENTS; i++) {
            ...
            if (age > MINING_SHARE_WINDOW_S * 2000) {
                memset(&s_clients[i], 0, sizeof(tollgate_mining_client_stats_t));
    ...
    tollgate_mining_client_stats_t *c = &s_clients[s_client_count];
    memset(c, 0, sizeof(tollgate_mining_client_stats_t));
    c->ip = client_ip;
...
void tollgate_core_mining_set_current_nbits(uint32_t nbits)
{
    ...
    s_current_difficulty = tollgate_core_mining_nbits_to_difficulty(nbits);
    s_current_hashprice = tollgate_core_mining_calc_hashprice(nbits);
