...
const ns1 = run(`dig +short random-test.example.com @${IP} 2>/dev/null`);
assert(ns1 && ns1.trim() === IP, 'DNS resolves arbitrary domain to AP IP');

// Test 6: No internet
console.log('\nTest 6: No internet before auth');
const ping1 = run('ping -c 1 -W 3 -I wlp59s0 1.1.1.1 2>/dev/null');
...
console.log('\nGranting access...');
run(`curl -s --connect-timeout 10 --max-time 20 ${API}/grant_access`);
...
const ns2 = run(`dig +short google.com @${IP} 2>/dev/null`);
assert(ns2 && !ns2.includes(IP) && ns2.trim().length > 0, 'DNS resolves to real IPs');

// Test 11: Internet
console.log('\nTest 11: Internet after auth');
const ping2 = run('ping -c 2 -W 3 -I wlp59s0 8.8.8.8');
...
const ping3 = run('ping -c 1 -W 3 -I wlp59s0 8.8.8.8 2>/dev/null');
