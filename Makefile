...
write-mining-config-vps:
	...
	@TMPDIR=$$(mktemp -d) && \
	echo '{"nsec":"$(NSEC_FOR_BOARD)","wifi_networks":[{"ssid":"$(WIFI_SSID)","password":"$(WIFI_PASSWORD)"}],"ap_password":"","mint_url":"$(MINT_URL)","accepted_mints":["$(MINT_URL)"],"price_per_step":21,"step_size_ms":60000,"display_enabled":false,"cvm_enabled":false,"sync_enabled":false,"wifistr_enabled":false,"local_relay_enabled":false,"mint_health_enabled":true,"mining":{"enabled":true,"payout_mode":"auto","stratum_host":"$(STRATUM_HOST)","stratum_port":$(STRATUM_PORT),"stratum_user":"tollgate_test","stratum_pass":"x","mining_port":3334,"faucet_url":"http://$(STRATUM_HOST):8083/mint/tokens","faucet_poll_interval_s":120}}' > "$$TMPDIR/config.json" && \
	echo "  Generating SPIFFS image..." && \
	python3 $(SPIFFSGEN) --page-size 256 --obj-name-len 32 --use-magic --use-magic-len $(SPIFFS_SIZE) "$$TMPDIR" "$$TMPDIR/spiffs.bin" && \
	echo "  Writing to flash..." && \
