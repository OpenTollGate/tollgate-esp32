...
"fetch('http://192.168.4.1:2121/').then(r=>r.json()).then(d=>{"
"if(d.tags){const p=d.tags.find(t=>t[0]==='price_per_step');if(p)priceEl.textContent=p[2]||'21';}"
"}).catch(()=>{priceEl.textContent='21';});"
...
"fetch('http://192.168.4.1:2121/',{method:'POST',body:token}).then(r=>{"
